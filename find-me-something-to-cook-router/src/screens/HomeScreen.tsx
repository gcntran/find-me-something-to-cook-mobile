...
const HomeScreen = () => {
    ...
    const handlePressRecipe = (recipe: Recipe) => {
        ...
        router.push(`/recipe/${recipe.id}`); //Navigate to /recipe/[id]
    ...
    const handleToggleSave = (recipe: Recipe) => {
        ...
        // Sync saved state across sections
        setSearchResults(prev => prev.map(r => r.id === recipe.id ? { ...r, saved: !r.saved } : r));
        setRandomRecipes(prev => prev.map(r => r.id === recipe.id ? { ...r, saved: !r.saved } : r));
        setRecentlyViewed(prev => prev.map(r => r.id === recipe.id ? { ...r, saved: !r.saved } : r));
    };
